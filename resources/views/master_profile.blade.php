        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
                ...
                @foreach($links as $link)
                    ...
                    <li style="font-size: 27px; color: #636B6F;/*font-family: sans-serif;*/" class="nav-item <?php if($testActive){echo $testActive ;} $testActive = false; ?>">
            ...
            <form class="form-inline my-2 my-lg-0">
                ...
                @if(Auth::check())
                    <li style="list-style: none;color:#5D6772;font-family: sans-serif;padding-left: 15px;">
                        ...
                        <a style="text-decoration: none;" href="view_profile?user_id={{Auth::user()->id}}">
            ...
            <ul class="navbar-nav mr-auto">
                ...
                @endforeach
                    <div style="position: relative">
                        <i class="fa fa-cart-plus " style="font-size: 36px; margin-left: 77px; color: #007BFF"></i>
                        <div style="position:absolute;width:35px;/*height:20px;*/bottom:25px;right:0;
                                    background-color:#bd2130; text-align:center;color:#FFF;vertical-align: auto;border-radius: 50%;
                                        " >
                            @if(!Auth::check())
                                0
                            @else
                                ...
                                <?php
                                $count = DB::select('SELECT * FROM `transactions` WHERE user_id=? && trans_type = ? '
                                    ,[Auth::user()->id , 1]);
                                echo count($count);
                                ?>
                            @endif
...
<div class="header miheader" style="background-image: url('images/caro/start-an-ecommerce-business.jpeg')">
    <div class="back ">
        <div class='container'>
            ...
            ?>
            <div class="" style="text-align: right; width:300px;height:300px;background-color: #2d5772;
              ;border-radius: 50%;
                            float:right;">
                        <img src="images/img/{{$img_url}}" style="width: 100%;height:100%;border-radius: 50%;">
                </div>
            <div style="clear: both;"></div>
        @if(Auth::check())
            @if(Auth::user()->id == $user_id)
                <form method="post" action="save_prfile/{{$user_id}}" style="float: right;">
                    {{ csrf_field() }}
                    <input type="file" name="img"  >
                    <button class="btn btn-outline-primary" style="padding-bottom: 0.075rem;margin-bottom: 10px;" type="submit">Save</button>
                </form>
            @else
